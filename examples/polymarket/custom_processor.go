...
func init() {
	...
	generated.LoadStateFromClickHouseFn = func(state *generated.State, ctx context.Context, httpPort int, blockNumber uint64) error {
		// Skip loading in test mode to avoid touching production database
		...
		dbName := "polymarket"
		if testDB := os.Getenv("CLICKHOUSE_DATABASE"); testDB != "" {
			dbName = testDB
		}

		queryCH := func(query string, scanRow func(dec *json.Decoder) error) error {
			...
			if resp.StatusCode != http.StatusOK {
				...
				return fmt.Errorf("clickhouse http error %d: %s", resp.StatusCode, string(bodyBytes))
			}
			dec := json.NewDecoder(resp.Body)
			for dec.More() {
				if err := scanRow(dec); err != nil {
					return err
				}
			}
			return nil
		...
		// Helper: parse interface{} to string
		...
		// Helper: parse interface{} to uint256.Int
		parseUint256 := func(v interface{}) (uint256.Int, error) {
			str := toString(v)
			p, err := uint256.FromDecimal(str)
			if err != nil {
		...
		// Helper: parse interface{} to decimal.Decimal
		parseDecimal := func(v interface{}) (decimal.Decimal, error) {
			str := toString(v)
			return decimal.NewFromString(str)
		}

		// 1. Load Conditions
		err := queryCH(fmt.Sprintf("SELECT id, oracle, question_id, outcome_slot_count, resolved, payouts FROM %s.memory_conditions ORDER BY block_number DESC, transaction_index DESC, log_index DESC LIMIT 1 BY id FORMAT JSONEachRow", dbName), func(dec *json.Decoder) error {
			var row struct {
				ID               string        `json:"id"`
			...
			for _, pVal := range row.Payouts {
				pInt, err := parseUint256(pVal)
				if err != nil {
					return fmt.Errorf("payout parse error: %w", err)
				}
				cond.Payouts = append(cond.Payouts, pInt)
			}
			state.Condition.Save(cond, generated.EventMeta{})
			return nil
		})
		if err != nil {
			return fmt.Errorf("load conditions from ClickHouse: %w", err)
		}

		// 2. Load User Positions
		err = queryCH(fmt.Sprintf("SELECT user, token_id, amount, avg_price, realized_pn_l, total_bought FROM %s.memory_user_positions ORDER BY block_number DESC, transaction_index DESC, log_index DESC LIMIT 1 BY user, token_id FORMAT JSONEachRow", dbName), func(dec *json.Decoder) error {
			var row struct {
				User        interface{} `json:"user"`
				TokenID     interface{} `json:"token_id"`
				Amount      interface{} `json:"amount"`
				AvgPrice    interface{} `json:"avg_price"`
				RealizedPnL interface{} `json:"realized_pn_l"`
				TotalBought interface{} `json:"total_bought"`
			}
			if err := dec.Decode(&row); err != nil {
				return err
			}
			amtDec, err := parseDecimal(row.Amount)
			if err != nil {
				return err
			}
			avgDec, err := parseDecimal(row.AvgPrice)
			if err != nil {
				return err
			}
			pnlDec, err := parseDecimal(row.RealizedPnL)
			if err != nil {
				return err
			}
			tbDec, err := parseDecimal(row.TotalBought)
			if err != nil {
				return err
			}
			pos := &generated.Position{
				User:        common.HexToAddress(toString(row.User)),
				TokenID:     common.HexToHash(toString(row.TokenID)),
				Amount:      fromDecimal(amtDec),
				AvgPrice:    fromDecimal(avgDec),
				RealizedPnL: fromDecimal(pnlDec),
				TotalBought: fromDecimal(tbDec),
			}
			state.Position.Save(pos, generated.EventMeta{})
			return nil
		})
		if err != nil {
			return fmt.Errorf("load user positions from ClickHouse: %w", err)
		}

		// 3. Load Neg Risk Events
		err = queryCH(fmt.Sprintf("SELECT id, question_count, question_ids FROM %s.memory_neg_risk_events ORDER BY block_number DESC, transaction_index DESC, log_index DESC LIMIT 1 BY id FORMAT JSONEachRow", dbName), func(dec *json.Decoder) error {
			var row struct {
				ID            string        `json:"id"`
			...
			for _, qidVal := range row.QuestionIDs {
				nr.QuestionIDs = append(nr.QuestionIDs, common.HexToHash(toString(qidVal)))
			}
			state.NegRiskEvent.Save(nr, generated.EventMeta{})
			return nil
		})
		if err != nil {
			return fmt.Errorf("load neg risk events from ClickHouse: %w", err)
		}

		// 4. Load Markets
		err = queryCH(fmt.Sprintf("SELECT id, question_count, question_ids FROM %s.memory_markets ORDER BY block_number DESC, transaction_index DESC, log_index DESC LIMIT 1 BY id FORMAT JSONEachRow", dbName), func(dec *json.Decoder) error {
			var row struct {
				ID            string        `json:"id"`
				QuestionCount uint32        `json:"question_count"`
				QuestionIDs   []interface{} `json:"question_ids"`
			...
			for _, qidVal := range row.QuestionIDs {
				m.QuestionIDs = append(m.QuestionIDs, common.HexToHash(toString(qidVal)))
			}
			state.Market.Save(m, generated.EventMeta{})
			return nil
		})
		if err != nil {
			return fmt.Errorf("load markets from ClickHouse: %w", err)
		}

		// 5. Load Fixed Product Market Makers
		err = queryCH(fmt.Sprintf("SELECT id, condition_id, collateral_token FROM %s.memory_fixed_product_market_makers ORDER BY block_number DESC, transaction_index DESC, log_index DESC LIMIT 1 BY id FORMAT JSONEachRow", dbName), func(dec *json.Decoder) error {
			var row struct {
				ID              interface{} `json:"id"`
				ConditionID     interface{} `json:"condition_id"`
				CollateralToken interface{} `json:"collateral_token"`
			}
			if err := dec.Decode(&row); err != nil {
				return err
			}
			fpmm := &generated.FixedProductMarketMaker{
				ID:              common.HexToAddress(toString(row.ID)),
				ConditionID:     common.HexToHash(toString(row.ConditionID)),
				CollateralToken: common.HexToAddress(toString(row.CollateralToken)),
			}
			state.FixedProductMarketMaker.Save(fpmm, generated.EventMeta{})
			return nil
		})
		if err != nil {
			return fmt.Errorf("load fixed product market makers from ClickHouse: %w", err)
		}
