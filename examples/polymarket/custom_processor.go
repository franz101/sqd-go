...
func hashCollectionKey(k collectionKey) uint64 {
	h := binary.LittleEndian.Uint64(k.parent[:8])
	h ^= binary.LittleEndian.Uint64(k.condition[:8])
	h ^= binary.LittleEndian.Uint64(k.index[:8])
	return h
}

func hashPositionKey(k positionKey) uint64 {
	h := binary.LittleEndian.Uint64(k.collateral[:8])
	h ^= binary.LittleEndian.Uint64(k.collection[:8])
	return h
}
...
func Process(state *generated.State, block *generated.ParsedBlock) error {
	...
	for ev := range block.EventsIter() {
		...
		case *generated.ExchangeV2OrderFilledV2:
			handleOrderFilledV2Values(state, e.Maker, e.Side, e.TokenID, e.MakerAmountFilled, e.TakerAmountFilled, e.EventMeta)
		case *generated.NegRiskExchangeV2OrderFilledV2:
			handleOrderFilledV2Values(state, e.Maker, e.Side, e.TokenID, e.MakerAmountFilled, e.TakerAmountFilled, e.EventMeta)
...
func ProcessProto(state *generated.State, block *generated.ProtoEventBlock) error {
	...
	var orderFilledV2Idx int
	var negRiskOrderFilledV2Idx int
	...
	for _, typ := range block.Sequence {
		...
		case generated.EventTypeNegRiskExchangeOrderFilled:
			...
			handleOrderFilledValues(state, ev.Maker(), ev.MakerAssetID(), ev.TakerAssetID(), ev.MakerAmountFilled(), ev.TakerAmountFilled(), ev.Meta())
		case generated.EventTypeExchangeV2OrderFilledV2:
			ev := block.ExchangeV2OrderFilledV2ProtoAt(orderFilledV2Idx)
			orderFilledV2Idx++
			handleOrderFilledV2Values(state, ev.Maker(), ev.Side(), ev.TokenID(), ev.MakerAmountFilled(), ev.TakerAmountFilled(), ev.Meta())
		case generated.EventTypeNegRiskExchangeV2OrderFilledV2:
			ev := block.NegRiskExchangeV2OrderFilledV2ProtoAt(negRiskOrderFilledV2Idx)
			negRiskOrderFilledV2Idx++
			handleOrderFilledV2Values(state, ev.Maker(), ev.Side(), ev.TokenID(), ev.MakerAmountFilled(), ev.TakerAmountFilled(), ev.Meta())
...
func handleOrderFilledV2Values(state *generated.State, maker common.Address, side, tokenID, makerAmountFilled, takerAmountFilled uint256.Int, meta generated.EventMeta) {
	if side.BitLen() > 1 {
		return
	}

	var makerAssetID, takerAssetID uint256.Int
	if side.IsZero() {
		// BUY: maker spends collateral and receives the outcome token.
		takerAssetID = tokenID
	} else {
		// SELL: maker spends the outcome token and receives collateral.
		makerAssetID = tokenID
	}
	handleOrderFilledValues(state, maker, makerAssetID, takerAssetID, makerAmountFilled, takerAmountFilled, meta)
}
...
func handleOrderFilledValues(state *generated.State, maker common.Address, makerAssetID, takerAssetID, makerAmountFilled, takerAmountFilled uint256.Int, meta generated.EventMeta) {
	...
	if !okM || !okT {
		// Unreachable overflow: amount too large for scale-18 conversion
		return
	}
	...
	if !baseAmount.IsZero() {
		...
		if price, ok = quoteAmount.Div(baseAmount, protomath.Decimal256Scale18); !ok {
			// Unreachable overflow: price calculation exceeds scale-18 range
			return
		}
	}
...
func handleFPMMFundingAdded(state *generated.State, ev *generated.FixedProductMarketMakerFPMMFundingAdded, fpmmAddr common.Address) {
	...
	if !okA || !okP {
		// Unreachable overflow: amount or price exceeds scale-18 range
		return
	}
	...
	if !okL {
		// Unreachable overflow: LP share cost calculation exceeds scale-18 range
		return
	}
	lpSharePrice, okLP := lpShareCost.Div(sharesMinted, scale)
	if !okLP {
		// Unreachable overflow: LP share price calculation exceeds scale-18 range
		return
	}
	updateUserPositionWithBuyD256(state, ev.Funder, posID, price, amount, protomath.Decimal256{}, ev.EventMeta)
...
func handleFPMMFundingRemoved(state *generated.State, ev *generated.FixedProductMarketMakerFPMMFundingRemoved, fpmmAddr common.Address) {
	...
	for i := uint8(0); i < 2; i++ {
		...
		if !okP || !okA || !okC {
			// Unreachable overflow: funding removal calculation exceeds scale-18 range
			return
		}
		nc, okN := tokensCost.Add(cost)
		if !okN {
			// Unreachable overflow: tokens cost accumulation exceeds scale-18 range
			return
		}
	...
	if hasShares {
		...
		if !okLP {
			// Unreachable overflow: LP share price division exceeds scale-18 range
			return
		}
...
func handlePositionSplit(state *generated.State, ev *generated.ConditionalTokensPositionSplit) {
	...
	if !okA {
		// Unreachable overflow: position split amount exceeds scale-18 range
		return
	}
	if amount.IsZero() {
		return
...
func handlePositionsMerge(state *generated.State, ev *generated.ConditionalTokensPositionsMerge) {
	...
	amount, okA := usdcRawToDec18(&ev.Amount)
	if !okA {
		// Unreachable overflow: positions merge amount exceeds scale-18 range
		return
	}
	if amount.IsZero() {
		return
...
func handlePositionsConverted(state *generated.State, ev *generated.NegRiskAdapterPositionsConverted) {
	...
	amount, okA := usdcRawToDec18(&ev.Amount)
	if !okA {
		// Unreachable overflow: positions converted amount exceeds scale-18 range
		return
	}
	scale := protomath.Decimal256Scale18
...
var collectionIndexWords = func() [64][32]byte {
	...
	for outcome := range words {
		words[outcome][31-outcome/8] = byte(1 << uint(outcome%8))
	}
	return words
}()
...
func getCollectionID(parentCollectionID common.Hash, conditionID common.Hash, indexSet *big.Int) common.Hash {
	indexWord := bigIntTo32Bytes(indexSet)
	key := collectionKey{
	...
	res := computeCollectionIDFixed(parentCollectionID, conditionID, indexWord)
	collectionCache.Store(key, res)
	return res
}

func getCollectionIDForOutcome(parentCollectionID common.Hash, conditionID common.Hash, outcome uint8) common.Hash {
	if int(outcome) >= len(collectionIndexWords) {
		return getCollectionID(parentCollectionID, conditionID, new(big.Int).Lsh(big.NewInt(1), uint(outcome)))
	}
	indexWord := collectionIndexWords[outcome]
	key := collectionKey{
		parent:    parentCollectionID,
		condition: conditionID,
		index:     indexWord,
...
func computeCollectionIDFixed(parentCollectionID common.Hash, conditionID common.Hash, indexWord [32]byte) common.Hash {
	hashResult := crypto.Keccak256Hash(conditionID[:], indexWord[:])
	odd := hashResult[0]&0x80 != 0

	var x1, y1, yy, one, three fp.Element
	x1.SetBytes(hashResult[:])
	one.SetOne()
	three.SetUint64(3)
	for {
		x1.Add(&x1, &one)
		yy.Square(&x1).Mul(&yy, &x1).Add(&yy, &three)
		if y1.Sqrt(&yy) != nil {
			break
		}
	}
	...
	var y1Big big.Int
	y1.BigInt(&y1Big)
	if (odd && y1Big.Bit(0) == 0) || (!odd && y1Big.Bit(0) == 1) {
		y1.Neg(&y1)
		y1.BigInt(&y1Big)
	}
	...
	var x1Big big.Int
	x1.BigInt(&x1Big)
	if parentCollectionID != (common.Hash{}) {
		x2 := new(big.Int).SetBytes(parentCollectionID[:])
		parentOdd := x2.Bit(254) == 1
		x2.And(x2, ctLow254Mask)

		yy2 := addMod(mulMod(x2, mulMod(x2, x2, ctP), ctP), ctB, ctP)
		y2 := sqrtModP(yy2)
		if (parentOdd && y2.Bit(0) == 0) || (!parentOdd && y2.Bit(0) == 1) {
			y2.Sub(ctP, y2)
		}
		if mulMod(y2, y2, ctP).Cmp(yy2) != 0 {
			panic("invalid parent collection ID")
		}
		x1Ptr, y1Ptr := ecAdd(&x1Big, &y1Big, x2, y2)
		x1Big.Set(x1Ptr)
		y1Big.Set(y1Ptr)
	}

	if y1Big.Bit(0) == 1 {
		x1Big.Xor(&x1Big, ctParityBit)
	}
	return common.BigToHash(&x1Big)
}

func computeCollectionIDLegacy(parentCollectionID common.Hash, conditionID common.Hash, indexWord [32]byte) common.Hash {
	indexSet := new(big.Int).SetBytes(indexWord[:])
