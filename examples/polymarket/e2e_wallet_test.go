...
func TestWallet0xf05b67Positions(t *testing.T) {
	...
	if err := recoveredProc.LoadFromDatabase(ctx, stats.lastBlock); err != nil {
...
func collectPositionsForWallet(t *testing.T, state *generated.State, wallet common.Address) []positionInfo {
	...
	// Ranging the hot ring alone silently drops positions that spilled to the cold
	// tier (or out of it) under memory pressure. Union the hot-resident tokens with
	// the durable token set from ClickHouse, then resolve each through the real
	// processor path state.Position.GetValue, whose authoritative gate consults the
	// eviction-proof negative (Bloom) filter and pulls any spilled survivor back from
	// ClickHouse. The Bloom filter keeps this O(1) for genuinely-new keys — only
	// seen-but-evicted positions pay a ClickHouse probe.
	seen := map[common.Hash]struct{}{}
	var tokens []common.Hash
	addToken := func(tok common.Hash) {
	...
	state.HotState.UserPositions.Range(func(_ generated.UserPositionsClockKey, pos generated.MemoryUserPosition) bool {
		if pos.User == wallet && !pos.Tombstone {
			addToken(pos.TokenID)
		}
		return true
	})
	for _, tok := range distinctUserTokens(t, state, wallet) {
		addToken(tok)
	}

	for _, tok := range tokens {
		pos, ok := state.Position.GetValue(wallet, tok)
		if !ok {
			continue
		}
		...
		positions = append(positions, positionInfo{
			tokenID:     tok,
			amount:      amt,
			avgPrice:    toDecimal(pos.AvgPrice),
			realizedPnL: rPnL,
			totalBought: tb,
		})
	}
...
// distinctUserTokens returns every token_id the wallet has a durable row for in
// ClickHouse — the authoritative position set, including entries evicted from the
// in-memory tiers. Returns nil when no store is wired (callers without ClickHouse
// fall back to hot-state enumeration only).
func distinctUserTokens(t *testing.T, state *generated.State, wallet common.Address) []common.Hash {
	t.Helper()
	if state.Store == nil || state.Store.Conn() == nil {
		return nil
	}
	var hexCol proto.ColStr
	var out []common.Hash
	q := fmt.Sprintf(
		"SELECT DISTINCT hex(token_id) AS th FROM `%s`.memory_user_positions WHERE user = unhex('%x')",
		state.Store.DB(), wallet.Bytes())
	if err := state.Store.Conn().Do(context.Background(), ch.Query{
		Body:   q,
		Result: proto.Results{{Name: "th", Data: &hexCol}},
		OnResult: func(_ context.Context, b proto.Block) error {
			for i := 0; i < b.Rows; i++ {
				out = append(out, common.HexToHash(hexCol.Row(i)))
			}
			return nil
		},
	}); err != nil {
		t.Fatalf("distinct user tokens: %v", err)
	}
	return out
}
...
func positionHoldingValue(state *generated.State, pos positionInfo) decimal.Decimal {
	if price, ok := resolvedPositionPayoutPrice(state, pos.tokenID); ok {
		// Unrealized (cashPnl) applies only to shares STILL HELD: amount*(payout-avgPrice).
		// Fully-closed positions (amount==0) contribute 0 here, matching the data-api,
		// where closed-positions carry realizedPnl only and no cashPnl — their P&L is
		// already captured in realizedPnL. Using totalBought would re-settle already-sold
		// shares and double-count (the source of the residual holdings inflation).
		return pos.amount.Mul(price.Sub(pos.avgPrice))
	}
	return decimal.Zero
}
...
func resolvedPositionPayoutPrice(state *generated.State, tokenID common.Hash) (decimal.Decimal, bool) {
	...
	collaterals := map[common.Address]struct{}{
		common.HexToAddress("0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174"): {}, // USDC (Polygon)
		negRiskWrappedCollateral: {}, // NegRisk wrapped collateral
		common.HexToAddress("0xAdA100Db00Ca00073811820692005400218FcE1f"): {}, // CtfCollateralAdapter
		common.HexToAddress("0xadA2005600Dec949baf300f4C6120000bDB6eAab"): {}, // NegRiskCtfCollateralAdapter
	}
	...
	state.HotState.Conditions.Range(func(_ generated.ConditionsClockKey, cond generated.MemoryCondition) bool {
		...
		// NegRiskAdapter conditions use a different position ID calculation
		isNegRisk := cond.Oracle == negRiskAdapterAddr
		for outcomeIndex := range cond.Payouts {
			if outcomeIndex >= 256 {
				break
			}

			var positionID uint256.Int
			if isNegRisk && outcomeIndex < 2 {
				// NegRisk positions use BabyJubJub curve computation
				positionID = getNegRiskPositionIDByCondition(cond.ID, uint8(outcomeIndex))
			} else {
				// CTF positions use collection ID
				indexSet := new(big.Int).Lsh(big.NewInt(1), uint(outcomeIndex))
				collectionID := getCollectionID(common.Hash{}, cond.ID, indexSet)

				// Try each collateral until we find a match
				for collateral := range collaterals {
					posID := getPositionID(collateral, collectionID)
					if tokenIDHash(posID) == tokenID {
						positionID = posID
						break
					}
				}
			...
			if !positionID.IsZero() && tokenIDHash(positionID) == tokenID {
...
func processDataFiles(ctx context.Context, t *testing.T, proc *generated.Processor, store *database.Store, dataDir string) (*processingStats, error) {
	...
	// The fixture overlaps: each of the wallet's activity blocks is captured BOTH in a
	// narrow per-wallet file and in a wide range file. Replaying every file applies
	// those blocks' fills twice (doubling totalBought/realizedPnL), so dedup by block
	// number and process each exactly once at its first (in-order) occurrence.
	...
	for _, filePath := range files {
		...
		err = jsonlParser.Parse(decompressed, func(block *parser.Block) error {
			if _, dup := seenBlocks[block.Header.Number]; dup {
				return nil
			}
			seenBlocks[block.Header.Number] = struct{}{}
			stats.blocks++
...
func globJSONLZstdFiles(dataDir string) ([]string, error) {
	var files []string
	for _, pattern := range []string{"*.jsonl.zstd", "*.jsonl.zst"} {
