...
# perf/prefetch branch — generic two-pass batch prefetch (`--prefetch`)

## PREFETCH — collapse per-key ClickHouse round-trips into one batch per block

**The cost.** On a hot-state miss the lazy path (`XxxState.GetValue`) resolves the
key *synchronously, one key at a time*: `Resolver.Queue(key)` then immediately
`Resolver.Resolve(...)` — a single-row `SELECT … WHERE (user,token_id) IN (…)`
ClickHouse round-trip (~1.9 ms each). A block that touches *K* distinct missing
keys pays **K serial round-trips**, interleaved with handler execution. In
resume/cursor mode against a populated ClickHouse (cold tier off or
non-authoritative) this miss-storm dominates the processor.

**The idea (user's framing: "dry run every Get, then on the second run fetch
all").** Process each block **twice**:

1. **Pass 1 — dry run (`recordMode`)**: dispatch the handlers, but every
   `GetValue` miss just **queues** the key (no per-key Resolve) and returns a miss,
   and **`Save` is suppressed** so the pass has no committed side effects. Cold-tier
   hits still promote real values into the ring for free.
2. **Resolve once**: `HotState.ResolveAllPending` drains every entity resolver in
   **one batched round-trip per entity** (the `IN (…)` list now carries the whole
   block's read-set). Not-found keys are cached as tombstones, so genuinely-new
   keys don't re-resolve in pass 2 either.
3. **Pass 2 — apply (`recordMode` off)**: dispatch for real against the now-warm
   cache; every read is a hot hit. Any key the dry run missed (handler branch
   divergence) still falls back to the lazy path, so prefetch is **observably
   equivalent to lazy** — pass 1 commits nothing and pass 2 keeps the fallback.

**Why dynamic, not static.** A static prefetch (the pre-existing, never-wired
`prefetchBlocksState`) needs the codegen to *derive* each key from event fields —
brittle and hand-special-cased per project (the polymarket `tokenID` logic was
hardcoded in the template). The dry run captures the **exact** read-set by running
the real handlers, regardless of how keys are derived. "Autogenerated, I don't
want to handle it": the two-pass wiring is emitted into generated
`CustomProcessing`/`CustomProcessingProto`; no per-project prefetch code.

**Implementation** (codegen templates, so it survives regeneration):
- `HotState`: `prefetchEnabled`/`recordMode` flags + `EnablePrefetch`,
  `SetRecordMode`, and a generic `ResolveAllPending` (one `Resolve` per entity).
- `state.go` `GetValue`: record-mode branch queues-without-resolving; `Save`:
  early-return while recording.
- `custom_processor.go` `CustomProcessing[Proto]`: the two-pass bracket, gated on
  `prefetchEnabled && store.Conn() != nil`.
- CLI `--prefetch` → `ingestion.Options.Prefetch` → `PrefetchProcessor.EnablePrefetch`.

**Measured benefit** (`internal/codegen/prefetch_runtime_test.go`,
`TestPrefetchRoundTripsCH`, against a real ClickHouse 26.5, 200 distinct seeded
positions, one block):

| Path | ClickHouse round-trips for 200 keys |
|---|---:|
| lazy (per-key Resolve) | **200** |
| `--prefetch` (one batched Resolve) | **1** |

**200× fewer round-trips**, with byte-identical values served (the test asserts
each `GetValue` returns the same balance both ways, and that warm reads after
prefetch add **zero** further round-trips). At ~1.9 ms/round-trip that is ~380 ms
→ a few ms for that block's resolution phase; the round-trip *count* is the
host-independent signal. `RoundTrips` is now surfaced on `BatchResolverMetrics`
for production observability.

**Tests.** Three hermetic mechanism tests (toggle plumbing; dry run queues the
read-set with **0** round-trips; `Save` suppressed while recording) +
the ClickHouse round-trip/equivalence test above. All generated from a fixture in
the codegen runtime harness, so they exercise the *real* generated code.

**Status**: implemented, flag-gated (off by default — costs a second, CPU-only
handler pass, so it only pays off when misses dominate), tested, and
round-trip-proven on real ClickHouse. The trade-off is explicit: best in
resume/cursor mode against a populated ClickHouse; neutral-to-negative on an
all-hot workload or under an authoritative cold tier (which already skips
ClickHouse). Next step if pursued: batch-level prefetch (collect the whole
multi-block batch's read-set before resolving) to drop from one round-trip
*per block per entity* to one *per batch per entity*.
