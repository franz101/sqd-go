...
// These cover the decoders wired into the parser codegen's EventMeta
// construction (AddressFromHex for the contract address, DecodeTopicHash for
// the block and transaction hashes). They must be byte-identical to the
// go-ethereum helpers they replaced, and allocation-free on canonical input —
// that allocation cut is the entire reason for the swap.
...
func TestDecodeTopicHashZeroAlloc(t *testing.T) {
	hash := "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"
	if n := testing.AllocsPerRun(1000, func() { hashSink = DecodeTopicHash(hash) }); n != 0 {
		t.Errorf("DecodeTopicHash allocates %.1f objects/call, want 0", n)
	}
