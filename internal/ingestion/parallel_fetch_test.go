...
// fakePortal serves JSONL block ranges like the SQD portal: it honors the pinned
...
func fakePortal(respCap uint64, garbageFrom, garbageTo uint64, delay time.Duration) *httptest.Server {
...
func TestParallelPrefetcherCancel(t *testing.T) {
	srv := fakePortal(2000, 0, 0, 40*time.Millisecond)
	defer srv.Close()
