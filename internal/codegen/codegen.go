...
func GenerateProject(project *config.Project) (string, error) {
	...
	// Build-tagged profiling toggle for the generated parser. The default file
	// compiles the per-event timing and [PARSE 10s] instrumentation out;
	// -tags sqdparseprof turns it on. Both must exist so exactly one definition of
	// parseProfilingEnabled is visible in any build.
	if err := os.WriteFile(filepath.Join(goOutDir, "parse_profiling.go"), []byte(parseProfilingOffFile), 0o644); err != nil {
		return "", err
	}
	if err := os.WriteFile(filepath.Join(goOutDir, "parse_profiling_enabled.go"), []byte(parseProfilingOnFile), 0o644); err != nil {
		return "", err
	}
...
// parseProfilingOffFile / parseProfilingOnFile are the build-tagged companions to
// the generated parser.go. They define parseProfilingEnabled, plus the
// _pinit/_pnow/_psince clock parser.go calls, so the per-line / per-event
// profiling clock and the [PARSE 10s] summary compile out of default builds and
// switch on under -tags sqdparseprof.
//
// The tsc import MUST live here, not in parser.go: parser.go is compiled into
// every build regardless of the tag, so an unconditional "import tsc" there
// would run tsc's package init() — a blocking ~2s TSC calibration — on every
// process start even with profiling disabled. Keeping the import inside this
// build-tagged file means the default (!sqdparseprof) build never references
// the tsc package at all, so it's never linked in and never pays that cost.
const parseProfilingOffFile = `// Code generated by sqd-go codegen; DO NOT EDIT.
...
const parseProfilingOnFile = `// Code generated by sqd-go codegen; DO NOT EDIT.
