...
type Config struct {
	...
	// ColdCacheMB bounds the shared off-heap Pebble block cache RAM for the cold
	// tier (one budget across every state entity). Env SQD_COLDCACHE_MB overrides
	// it; unset falls back to RAM/8 clamped. On-disk Pebble growth is uncapped.
