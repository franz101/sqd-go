...
func batchFetchMissing(ctx context.Context, state *generated.State, store *database.Store, slot *generated.BlockEventsSlot) error {
	// Stage 1: Gather and resolve Conditions, FPMMs, and NegRiskEvents
	for _, ev := range slot.ConditionalTokensConditionPreparations {
		state.HotState.ConditionsResolver.Queue(generated.ConditionsClockKey{ID: ev.ConditionID})
	}
	...
	for _, ev := range slot.ConditionalTokensPositionSplits {
		state.HotState.ConditionsResolver.Queue(generated.ConditionsClockKey{ID: ev.ConditionID})
		state.HotState.FixedProductMarketMakersResolver.Queue(generated.FixedProductMarketMakersClockKey{ID: ev.Stakeholder})
	}
	...
	for _, ev := range slot.FixedProductMarketMakerFactoryFixedProductMarketMakerCreations {
		if len(ev.ConditionIds) > 0 {
			state.HotState.ConditionsResolver.Queue(generated.ConditionsClockKey{ID: ev.ConditionIds[0]})
		}
		state.HotState.FixedProductMarketMakersResolver.Queue(generated.FixedProductMarketMakersClockKey{ID: ev.FixedProductMarketMaker})
	}
	for _, ev := range slot.FixedProductMarketMakerFPMMBuys {
		state.HotState.FixedProductMarketMakersResolver.Queue(generated.FixedProductMarketMakersClockKey{ID: ev.ContractAddress})
	}
	...
	for _, ev := range slot.NegRiskAdapterMarketPrepareds {
		state.HotState.NegRiskEventsResolver.Queue(generated.NegRiskEventsClockKey{ID: ev.MarketID})
	}
	...
	// Stage 2: Queue all Positions using now-resolved structures
	...
	queuePosition := func(user common.Address, tokenID uint256.Int) {
		state.HotState.UserPositionsResolver.Queue(generated.UserPositionsClockKey{
			User:    user,
			TokenID: tokenIDHash(tokenID),
		})
	}
	...
	for _, ev := range slot.ExchangeOrderFilleds {
		...
		queuePosition(ev.Maker, tokenID)
	}
	...
	for _, ev := range slot.FixedProductMarketMakerFPMMFundingAddeds {
		...
		if fpmm != nil {
			...
			posID := getFixedProductMarketMakerPositionID(fpmm, outcomeIndex)
			queuePosition(ev.Funder, posID)
			queuePosition(ev.Funder, uint256FromAddress(fpmm.ID))
		}
	}
	for _, ev := range slot.FixedProductMarketMakerFPMMFundingRemoveds {
		fpmm := getFPMM(ev.ContractAddress)
	...
	for _, ev := range slot.ConditionalTokensPositionSplits {
		for outcomeIndex := uint8(0); outcomeIndex < 2; outcomeIndex++ {
			...
			collID := getCollectionID(common.Hash{}, ev.ConditionID, indexSet.ToBig())
			posID := getPositionID(ev.CollateralToken, collID)
			queuePosition(ev.Stakeholder, posID)
		}
	}
	for _, ev := range slot.ConditionalTokensPositionsMerges {
		for outcomeIndex := uint8(0); outcomeIndex < 2; outcomeIndex++ {
	...
	for _, ev := range slot.NegRiskAdapterPositionSplits {
		posIDYes := getNegRiskPositionIDByCondition(ev.ConditionID, 0)
		posIDNo := getNegRiskPositionIDByCondition(ev.ConditionID, 1)
		queuePosition(ev.Stakeholder, posIDYes)
		queuePosition(ev.Stakeholder, posIDNo)
	}
	for _, ev := range slot.NegRiskAdapterPositionsMerges {
		posIDYes := getNegRiskPositionIDByCondition(ev.ConditionID, 0)
		posIDNo := getNegRiskPositionIDByCondition(ev.ConditionID, 1)
	...
	for _, ev := range slot.ConditionalTokensPayoutRedemptions {
		for i := uint8(0); i < 2; i++ {
			...
			collID := getCollectionID(common.Hash{}, ev.ConditionID, indexSet.ToBig())
			posID := getPositionID(ev.CollateralToken, collID)
			queuePosition(ev.Redeemer, posID)
		}
	}
	for _, ev := range slot.NegRiskAdapterPayoutRedemptions {
		for i := uint8(0); i < 2; i++ {
...
func main() {
	...
	for _, bf := range bfs {
		...
		err = jsonParser.Parse(decompressed, func(block *parser.Block) error {
			...
			if err := batchFetchMissing(ctx, state, store, slot); err != nil {
				log.Fatalf("failed to batch fetch missing entities for block %d: %v", block.Header.Number, err)
			}
			...
			if *commitInterval > 0 && blockCount%*commitInterval == 0 {
				...
				if err := state.Commit(ctx, store); err != nil {
					log.Fatalf("failed to commit state to ClickHouse at block %d: %v", block.Header.Number, err)
				}
