...
func main() {
	...
	chDB := flag.String("ch-db", "polymarket_debug", "ClickHouse database name")
	...
	startTime := time.Now()
	var chSetupTime, recoveryTime, decompressionTime, parsingTime, processTime, commitTime time.Duration

	tCHSetupStart := time.Now()
	// Clear ClickHouse database if requested
	if *clearFlag {
	...
	// Initialize ClickHouse Native connection
	...
	// Apply schema and custom schema
	...
	// Recover existing hotstate from ClickHouse if requested
	if *recoverFlag {
	...
	// Pre-populate missing bootstrap entities if starting fresh/empty
	...
	for _, bf := range bfs {
		...
		err = jsonParser.Parse(decompressed, func(block *parser.Block) error {
			...
			// Periodically commit hotstate to ClickHouse (if requested, else 0 skips periodic commits)
	...
	// Fetch targetUser in-memory results
...
", totalPnL.Div(decimal.NewFromFloat(1e18)).String())
	...
	// Verify by querying ClickHouse custom table directly!
