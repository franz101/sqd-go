...
func main() {
	...
	// Capture interrupt signals for graceful shutdown and final stats printing
	...
	// Atomic counter for tracking processed blocks in a non-blocking, thread-safe way
	var blocksCounter atomic.Uint64
	startTime := time.Now()

	// Start background goroutine to print stats every 10 seconds
	go func() {
	...
	for {
		...
		// Decompress raw bytes to extract the last block number
		...
		// Save the raw Zstd bytes directly to output file
		...
		log.Printf("Successfully saved chunk to %s (blocks %d to %d)", outFilePath, fromBlock, lastBlock)

		// Increment the atomic counter by the number of blocks processed in this chunk
